...
function displayQuestion(i) {
    ...
    const url = "https://mtn-gaming.herokuapp.com/quatreimagesunmotquestionlist";
    fetch(url)
        .then(response => response.json())
        .then(data => {
            if (i <= 14) {
                document.getElementById("reference-question").innerHTML = `Image N°${data[i].id}<br>Mot en<br> ${data[i].indice}`;
                document.getElementById("question").src = `./assets/images/quatre_images_un_mot/${data[i].id}.PNG`;
