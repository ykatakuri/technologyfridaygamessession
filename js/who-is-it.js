...
function displayQuestion(i) {
    i = index; 
    const url = "https://mtn-gaming.herokuapp.com/whoisitimagelist";
    fetch(url)
        .then(response => response.json())
        .then(data => {
            if (i <= 14) {
                document.getElementById("reference-question").innerHTML = `Image N°${data[i].id}`;
                document.getElementById("question").src = data[i].modifiedImagePath;
...
function displayAnswer() {
    ...
    fetch(url)
        .then(response => response.json())
        .then(data => {   
            document.getElementById("question").src = data[index].realImagePath;
